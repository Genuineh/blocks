...
每次只推进一个清晰边界内的变化，避免同时改动契约模型、执行模型和前端壳层。优先通过小步增量方式推进，始终让已有测试和示例应用保持可回归。
...
- 桌面端用 `Tauri + TypeScript`，只做最小可视化和调试入口，不承担核心业务逻辑。
...
这个结构有一个硬约束：`cli` 和未来的 `tauri` 只是入口层，不能反向承载核心契约、编排或运行逻辑；核心复杂度必须收敛在可测试的 Rust crate 中。
...
建议 Tauri 只调用 Rust 后端命令，不在前端复制业务逻辑。
...
进入本里程碑前，先确认 CLI 已经是唯一可信入口，避免前端反向定义核心行为。
